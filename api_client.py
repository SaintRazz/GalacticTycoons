...
def get_commodity_price(commodity_name, api_key, desired_quantity=1):
    ...
    material = next(
        (item for item in materials if item.get("id") == match.get("matId")),
        None,
    )
...
def _find_named_item(items, requested_name, name_key):
    """Find an item by name, preferring exact matches over partial matches."""
    ...
    return exact_match or next(
        (
            item
            for item in items
            if requested in str(item.get(name_key, "")).casefold()
        ),
        None,
    )
...
def _build_production_analysis(
    ...
    """Build direct recipe, dependency, and basal-resource analysis."""
    ...
    for recipe in recipes:
        ...
        recipes_by_output.setdefault(output_id, []).append(recipe)
...
def _amount(item):
    """Read an amount field from either 'am' or 'a'."""
...
def _aggregate_basal_resources(
    ...
    """Reduce a production chain to its final basal/raw resource requirements."""
    ...
    for material_id, quantity in totals.items():
        ...
        unit_price = (
            ...
            if isinstance(unit_price_cents, (int, float))
...
def _add_basal_requirement(
    ...
    """
    Recursively decompose a material until a basal/raw resource is reached.

    A material is treated as basal if:
    - the game marks it as an extracted resource,
    - no production recipe exists for it, or
    - all known recipes for it have no inputs.
    """
    ...
    # FIX:
    # Previously, only materials with no recipe at all were treated
    # as basal. Some raw/extracted materials have a recipe entry with
    # no inputs, so they were reached by recursion but never added to
    # the totals dictionary.
    if (
        material.get("source") == 1
        or not material_recipes
        or all(not recipe.get("inputs") for recipe in material_recipes)
    ):
        totals[material_id] = totals.get(material_id, 0) + quantity
        return

    # If multiple recipes exist, the current implementation uses
    # the first one for basal-resource aggregation.
    ...
    # Ceiling division:
    # determine how many complete production batches are required.
...
def _build_dependency_recipes(
    ...
    """Build the nested recipe tree used to display production dependencies."""
    ...
    for recipe in recipes_by_output.get(material_id, []):
        ...
        dependency_recipes.append({
            ...
            "required_technology": recipe.get(
                "reqTech",
                0,
            ),
